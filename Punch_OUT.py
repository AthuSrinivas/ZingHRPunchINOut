...
try:
    ...
    response_ui = win32ui.MessageBox("Please check if zing_credential.txt file is in the same folder as the EXE!", "Punch OUT", win32con.MB_OK)
    exit()
...
data = {"Subscription":"embitel","Type":"PUNCHOUT","UserId":Emp_id,"Empcode":Emp_id,"Password":encrypted_password,"SyncVal":random_key}
try:
# Send POST request with custom headers and data (if any)
    ...
    if response.status_code == 200:
        ...
        response_ui = win32ui.MessageBox(Table1['MSG'], "Punch Out", win32con.MB_OK)
    else:
        response_ui = win32ui.MessageBox("Punch-Out Failed!\nPlease Check for the following:\n  1.Credentials are Properly given in zing_credentials.txt file.\n  2.If you are connected properly to internet.", "Punch Out", win32con.MB_OK)
except:
    response_ui = win32ui.MessageBox("Please check if you are connected to the Internet!", "Punch Out", win32con.MB_OK)
